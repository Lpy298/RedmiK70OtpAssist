...
data class PageSnapshot(val page:PageType,val width:Int,val height:Int,val items:List<UiText>,val signature:String=items.joinToString("|"){it.merged}){fun exact(v:String)=items.filter{i->i.labels.any{it==norm(v)}};fun contains(v:String)=items.filter{it.merged.contains(norm(v))}}
