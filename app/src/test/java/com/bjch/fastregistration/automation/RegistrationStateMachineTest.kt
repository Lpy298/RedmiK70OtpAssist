...
class RegistrationStateMachineTest {
 private val cfg=GrabConfig("TEST_DOCTOR_A","TEST_PATIENT_A",LocalDate.of(2026,9,10))
 ...
 private fun page(type:PageType,vararg i:UiText,signature:String=type.name+":"+i.joinToString("|"){it.merged})=PageSnapshot(type,706,1568,i.toList(),signature)
 ...
 @Test fun onlyTargetDoctorSameRowPositiveRemaining(){val m=RegistrationStateMachine(cfg);m.start();val selected=u("10",100,300,140,350,selected=true);m.observe(page(PageType.DEPARTMENT_SLOTS,selected,u("TEST_DOCTOR_A",100,700,180,740),u("剩余 0",550,700,640,740),u("TEST_DOCTOR_B",100,800,180,840),u("剩余 9",550,800,640,840),u("剩余 7",550,700,640,740)),1);val o=m.observe(page(PageType.DEPARTMENT_SLOTS,selected,u("TEST_DOCTOR_A",100,700,180,740),u("剩余 0",450,700,530,740),u("剩余 7",550,700,640,740),u("TEST_DOCTOR_B",100,800,180,840),u("剩余 9",550,800,640,840)),2);val c=o.action as MachineAction.Click;assertEquals("剩余 7",c.target.text)}
 @Test fun forbiddenStatusesNeverClicked(){for(v in listOf("约满","候补","即将放号","剩余 0")){val m=RegistrationStateMachine(cfg);m.start();val d=u("10",100,300,140,350,selected=true);m.observe(page(PageType.DEPARTMENT_SLOTS,d,u("TEST_DOCTOR_A",100,700,180,740),u(v,550,700,640,740)),1);val o=m.observe(page(PageType.DEPARTMENT_SLOTS,d,u("TEST_DOCTOR_A",100,700,180,740),u(v,550,700,640,740)),2);assertFalse(o.action is MachineAction.Click)}}
 @Test fun patientMustMatchExactly(){val m=RegistrationStateMachine(cfg);m.start();val o=m.observe(page(PageType.PATIENT_PICKER,u("TEST_PATIENT_B",100,500,200,550)),1);assertEquals(EngineState.ERROR,o.state)}
 ...
 @Test fun completeScreenshotReplay(){
  ...
  assertTrue(feed(page(PageType.PATIENT_PICKER,u("TEST_PATIENT_A",200,580,330,650))).action is MachineAction.Click)
  ...
  feed(page(PageType.DEPARTMENT_SLOTS,u("10",100,300,140,350,selected=true),u("TEST_DOCTOR_A",100,700,180,740),u("剩余 7",550,700,640,740),signature="selected"))
  assertTrue(feed(page(PageType.DEPARTMENT_SLOTS,u("10",100,300,140,350,selected=true),u("TEST_DOCTOR_A",100,700,180,740),u("剩余 7",550,700,640,740),signature="selected")).action is MachineAction.Click)
  ...
  assertTrue(feed(page(PageType.BOOKING_CONFIRM,u("TEST_DOCTOR_A",20,300,100,340),u("TEST_PATIENT_A",20,350,100,390),u("2026年09月10日",20,400,200,440),u("确认挂号",250,1300,460,1380))).action is MachineAction.Click)
